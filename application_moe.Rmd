...
params:
  repo_url: 'https://github.com/lucy-schick/fish_passage_fraser_2025_permit'
  report_url: 'https://lucy-schick.github.io/fish_passage_fraser_2025_permit/'
...
# Build the link to the kml file
link_kml <- fs::path(params$repo_url, '/raw/main/mapping/', paste0("sites_", project_region, "_", permit_year, "_", format(Sys.Date(), '%Y%m%d'), "_kml.zip"))
...
`r if (gitbook_on == FALSE) knitr::asis_output(paste0("This permit application can also be viewed online [at this link](", params$report_url, ")."))` A summary of sites proposed for assessment, including historic fish presence records from FISS, is provided in Tables \@ref(tab:tab-sites1) to \@ref(tab:tab-sites3). Fish species known to occur within each watershed are summarized in Table \@ref(tab:tab-fish). An overview map showing potential sample locations is presented in Figure \@ref(fig:map). A KML file of all sites is attached to the application and can also be downloaded [at this link](`r paste0(link_kml)`). The KML includes detailed site-specific information accessible by clicking on each location, with brief summaries of background reports where available.
...
# Build the gpx file name with today's date
today_date <- format(Sys.Date(), "%Y%m%d")
prefix <- paste0("sites_", project_region, "_", permit_year, "_")
path_gps <- paste0(prefix, today_date, ".gpx")

# Delete old gpx files that are not today's file
gpx_files <- fs::dir_ls("mapping", regexp = paste0("^mapping/", prefix, ".*\\.gpx$"))

files_to_delete <- gpx_files[!fs::path_file(gpx_files) %in% path_gps]
...
# Delete old kml files that are not today's file
path_kml <- paste0(prefix, today_date, ".kml")
kml_files <- fs::dir_ls("mapping", regexp = paste0("^mapping/", prefix, ".*\\.kml$"))
files_to_delete <- kml_files[!fs::path_file(kml_files) %in% path_kml]
fs::file_delete(files_to_delete)
...
zip::zipr(
  zipfile = paste0("mapping/", prefix, today_date, "_kml.zip"),
  files = paste0("mapping/", path_kml)
)
